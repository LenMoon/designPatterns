...
- 模板方法模式(template pattern):在一个方法中定义了一个算法的骨架，而将一些步骤延迟到子类中，模板方法使得子类可以在不改变算法结构的情况下，重新定义算法中的某些步骤
...
1. 有两个类Coffee和Tea
2. Caffee的方法有，煮沸水，冲泡咖啡，倒入杯中，加入牛奶和糖块
3. Tea的方法有，煮沸水，冲泡茶、倒入杯中、加入柠檬汁
4. 将上述俩个类进行抽象去除重复代码

*设计:*

1. 首先第一步我们分析出它们都有煮沸水，和倒入杯中的方法，那么建立一个抽象父类，将这两个公共方法实现等待子类继承
2. 然后我们发现他们的步骤也很相识，那么就将冲茶叶和冲咖啡都泛化为冲的动作，将加牛奶和糖块和加柠檬汁都泛化为添加调料的方法，然后在父类中以抽象的形式存在。
3. 接着父类就可以定义这一整套流程煮沸水、冲、倒入杯中、加入调料供子类使用
4. 作为子类只需要实现父类中的抽象方法，就可以使用了。
5. 以上就是模板模式
6. 模板模式还可以加入一些钩子，例如可以定义一个默认为true的withCodiments方法表示默认加配料，子类可以选择覆盖和不覆盖钩子方法。
7. 通过模板方法，我们将冲泡的算法封装，子类只需要实现特定的几个步骤就可以完成任务。

![模板模式](image/template/template.jpg)

>上面是一个简单的模板模式在实际使用中模板模式也有很多变种，例如Jdk的数组排序，需要数组中的元素实现Compareable接口的CompareTo方法，就是模板方法的一个变种
首先它封装了算法，只需要传进去的值（不是子类了）实现算法的某个部分。